...
defmodule(Assignment.Messages.TodoTask) do
  @moduledoc(false)
  (
    ...
    (
      ...
      [defp(encode_task_operation(acc, msg)) do
        ...
        if(field_value == :ADD) do
          ...
          [acc, "\b", field_value |> Assignment.Messages.TodoTask.TodoTaskOperation.encode() |> Protox.Encode.encode_enum()]
        end
    ...
    (
      ...
      (
        ...
        def(decode!(bytes)) do
          parse_key_value(bytes, struct(Assignment.Messages.TodoTask))
        end
      )
      (
        ...
        defp(parse_key_value(bytes, msg)) do
          {field, rest} = case(Protox.Decode.parse_key(bytes)) do
            ...
            {1, _, bytes} ->
              {value, rest} = Protox.Decode.parse_enum(bytes, Assignment.Messages.TodoTask.TodoTaskOperation)
    ...
    def(defs()) do
      %{1 => {:task_operation, {:default, :ADD}, {:enum, Assignment.Messages.TodoTask.TodoTaskOperation}}, 2 => {:task_uuid, {:default, ""}, :string}, 3 => {:currency_pair, {:default, ""}, :string}, 4 => {:from_unix_ts, {:default, 0}, :int32}, 5 => {:until_unix_ts, {:default, 0}, :int32}}
    end
    @spec(defs_by_name() :: %{required(atom) => {non_neg_integer, Protox.Types.kind(), Protox.Types.type()}})
    def(defs_by_name()) do
      %{currency_pair: {3, {:default, ""}, :string}, from_unix_ts: {4, {:default, 0}, :int32}, task_operation: {1, {:default, :ADD}, {:enum, Assignment.Messages.TodoTask.TodoTaskOperation}}, task_uuid: {2, {:default, ""}, :string}, until_unix_ts: {5, {:default, 0}, :int32}}
    end
