...
defmodule(AssignmentMessages.TaskResponse) do
  @moduledoc(false)
  (
    defstruct(task_result: :COMPLETE, todo_task_uuid: "", __uf__: [])
    (
      ...
      def(encode!(msg)) do
        [] |> encode_task_result(msg) |> encode_todo_task_uuid(msg) |> encode_unknown_fields(msg)
      end
      []
      ...
      end, defp(encode_todo_task_uuid(acc, msg)) do
        field_value = msg.todo_task_uuid()
        if(field_value == "") do
          acc
        else
    ...
    def(defs()) do
      %{1 => {:task_result, {:default, :COMPLETE}, {:enum, AssignmentMessages.TaskResponse.Result}}, 2 => {:todo_task_uuid, {:default, ""}, :string}}
    end
    @spec(defs_by_name() :: %{required(atom) => {non_neg_integer, Protox.Types.kind(), Protox.Types.type()}})
    def(defs_by_name()) do
      %{task_result: {1, {:default, :COMPLETE}, {:enum, AssignmentMessages.TaskResponse.Result}}, todo_task_uuid: {2, {:default, ""}, :string}}
    end
    ...
    [@spec(default(atom) :: {:ok, boolean | integer | String.t() | float} | {:error, atom}), [def(default(:task_result)) do
      {:ok, :COMPLETE}
    end, def(default(:todo_task_uuid)) do
      {:ok, ""}
